...
public class ScheduleSQL {
	...
	public ScheduleArray FindScheduleArrayByJobId(int jobId){
		...
		try {
			...
			ResultSet rs2 = dbconn.select("Id,employeeId,jobId,itemId,competencyId,startTimeDate,duration FROM schedule where jobId = " + jobId);
	...
	public ScheduleArray FindScheduleArrayByDateCompetencyId(Date startTimeDate,int competencyId){
		...
		try {
			...
			ResultSet rs2 = dbconn.select("Id,employeeId,jobId,itemId,competencyId,startTimeDate,duration FROM schedule where competencyId = " + competencyId + " AND startTimeDate = '" + startTimeDate + "'" );
	...
	public Schedule FindScheduleById(int Id){
		...
		try {
			dbconn.connect();
			ResultSet rs2 = dbconn.select("Id,employeeId,jobId,itemId,competencyId,startTimeDate,duration FROM schedule WHERE id = " + Id);
			rs2.next();
			System.out.println("Id=" + rs2.getInt("Id"));
			System.out.println("employeeId=" + rs2.getString("employeeId"));
			System.out.println("jobId=" + rs2.getString("jobId"));
	...
	public String UpdateSchedule(Schedule schedule){
		...
		try {
			dbconn.connect();
			dbconn.update("UPDATE schedule SET employeeId = " + schedule.getEmployee().getId() +  ",jobId = " + schedule.getJob().getId() +  ",itemId = " + schedule.getItem().getId() +  ",competencyId = " + schedule.getCompetency().getId() +  ",startTimeDate = '" + dbconn.convertDateTime(schedule.getStartTimeDate()) +  "',duration = '" + dbconn.convertDateTime(schedule.getDuration()) +  "' WHERE Id = " + schedule.getId());
			dbconn.close();
